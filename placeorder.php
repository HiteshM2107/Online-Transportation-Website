...
$email=$value["Email"];
	...
	<div>
		...
		$l=$_REQUEST["weight"];
		$abc=mysqli_connect("localhost","root","","ict");
		$sour="select Distance from route where Place='".$s."'";
		$dest="select Distance from route where Place='".$d."'";
		$s1=mysqli_query($abc,$sour);
		$d1=mysqli_query($abc,$dest);
		echo '<font color="green" size=7> Thank you for trading with us! </font><br><br>';
		echo "<u><font size=5>Please review your Order once.</font></u><br><br>";
		echo "<u>Source</u> - ".$s."<br>";
		echo "<u>Destination</u> - ".$d."<br>";
		echo "<u>Load</u> - ".$l." tonnes <br>";
		$T="$l tonnes from $s to $d";
		$t="update data set Orders='".$T."' where Email='".$_SESSION["Name"]."'";
		$g=mysqli_query($abc,$t);
